...
def main(argv=None):
    ...
    parser.add_argument("model_name", type=str, choices=("gpt2", "gpt2-medium", "gpt2-large", "distilgpt2"),
                        help="Pretrained model to use.")
    parser.add_argument("initial_text", type=str, help="Initial text")
    ...
    parser.add_argument("-k", "--top_k", type=int, help="If specified, only the top k tokens are considered.")
    parser.add_argument("-v", "--verbose", action="store_true", help="If true then verbose")
    ...
    our_param = ["vocab_size", "n_layer", "n_embd", "n_head", "n_positions", "attn_pdrop", "embd_pdrop", "resid_pdrop", "layer_norm_epsilon"]

    config_ours = {k: getattr(config_official, k) for k in our_param}
    ...
    for step in range(args.steps):
        new_token_ix = generate_token(model_ours, token_ixs, sample=args.sample, top_k=args.top_k, temperature=args.temperature)
